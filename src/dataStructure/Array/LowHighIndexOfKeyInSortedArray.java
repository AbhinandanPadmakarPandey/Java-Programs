 ...
 *
 * Runtime complexity #
 * Since we are using binary search, the runtime complexity is logarithmic, O(log n).
 *
 *     - Even though we do the binary search twice, asymptotic runtime complexity is still O(log n).
 *
 * Memory complexity #
 * The memory complexity is constant, O(1) since no extra storage is being used.
 *
 * Note: If the binary search was implemented recursively, there would be implicit O(log n)
 * memory usage on the function call stack. We, however, will be focusing on the iterative solution here.
 *
 * Linearly scanning the sorted array for low and high indices are highly inefficient since our
