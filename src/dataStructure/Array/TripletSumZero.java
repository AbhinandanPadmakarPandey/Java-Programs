 ...
 * one number at a time. Let’s say during our iteration we are at number ‘X’, so we need to find ‘Y’
 * and ‘Z’ such that X+Y+Z==0X + Y + Z == 0X+Y+Z==0. At this stage, our problem translates into finding a
 * pair whose sum is equal to “−X-X−X” (as from the above equation Y+Z==−XY + Z == -XY+Z==−X).
 *
